...
\begin{frame}{Timeline of Key Embedding Models}
\small
\begin{itemize}
    \item \textbf{Word2Vec} -- 2014\\
    \hspace{1em}\textit{Introduced efficient neural word embeddings.}
    \vspace{0.5em}
    \item \textbf{Transformers} -- 2017\\
    \hspace{1em}\textit{Vaswani et al. introduced the Transformer architecture, enabling powerful sequence modeling.}
    \vspace{0.5em}
    \item \textbf{BERT} -- 2018\\
    \hspace{1em}\textit{Devlin et al. introduced BERT, a bidirectional transformer for language understanding.}
    \vspace{0.5em}
    \item \textbf{ChatGPT-3.5} -- 2022\\
    \hspace{1em}\textit{OpenAI released ChatGPT-3.5, a large language model for conversational AI.}
\end{itemize}
\end{frame}
...
\subsection{Non-Contextual \\Embeddings}
...
\begin{frame}{Word2Vec: Continous Bag of Words (CBOW) vs skipgram}
	...
	\begin{block}{CBOW: Predict a word given the words in its context}
		Dataset:
	...
	\begin{block}{skipgram: Predict the context words given the target word}	
...
\begin{frame}{From static to contextual embeddings}
\small
\begin{itemize}
    \item \textbf{Word2Vec:}
    \begin{itemize}
        \item Learns a single fixed vector for each word.
        \item Embeddings are based on neighboring words.
        \item Cannot distinguish between different meanings of a word in different sentences.
    \end{itemize}
    \vspace{0.5em}
    \item \textbf{Transformers:}
    \begin{itemize}
        \item Dynamically compute word representations based on the entire sentence.
        \item Each word's embedding can change depending on surrounding words.
        \item Attention mechanism allows the model to focus on relevant words anywhere in the input.
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Training contextual word embeddings}
	\begin{itemize}
...
\begin{frame}{Example Visualization}
    Ongoing work within the TWON project, visualization credit to Simon M"unker, Trier University
\end{frame}
...
\begin{frame}{Convolutional networks}
	...
	\end{itemize}
	This means that we now explicitly take into account \emph{the temporal structure} of a sentence.
\end{frame}
...
\begin{frame}{Attention Mechanism}
    ...
    \begin{center}
        \Large
        ``\textit{The animal didn't cross the street because \textcolor{blue}{it} was too tired}''
    \end{center}
    ...
    \begin{figure}[h]
        \centering
        \includegraphics[width=0.5\textwidth]{pictures/attn_mech.png}
        \caption*{\small Source: Jay Alammar - "The Illustrated Transformer" \\ \url{https://jalammar.github.io/illustrated-transformer/}}
    \end{figure}
	...
	\begin{block}{BERT: Bidirectional Encoder Representations from Transformers \parencite{BERT}}
		\begin{itemize}[<+->]
			...
			\begin{enumerate}
				...
				\item natural language inference tasks (token prediction)
		\end{enumerate}
	\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{How to use}
	\begin{block}{HuggingFace}
		\begin{itemize}[<+->]
			...
			\item `HuggingFace`'s API works well with libraries such as `SentenceTransformer` `TensorFlow`, `Keras`, and `PyTorch`. 
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{An example from our colleagues research}
...
\tiny{Kroon, A.C., Van der Meer, G.L.A., Jonkman, J.G.F., \&Trilling, D. (2021): Guilty by Association: Using Word Embeddings to Measure Ethnic Stereotypes in News Coverage. \emph{Journalism \& Mass
...
%skip AEM example, instead more time to work on own data
