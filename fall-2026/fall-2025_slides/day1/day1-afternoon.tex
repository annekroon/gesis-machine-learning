...
\newcommand{\fixedrow}[1]{%
  \begin{minipage}[c][0,8cm][c]{\linewidth}\centering #1
  \end{minipage}}
...
\subsection{Machine \emph{Learning}: How to teach it?}

\begin{frame}{Teaching a toddler}
    \begin{itemize}[<+->]
        \item point it out
        \item explain in other words
        \item from the context: You shall know a word by the company it keeps (Firth)
        \item we need something even simpler
    \end{itemize}
\end{frame}
...
\toprule
\textbf{Human Learning} & \textbf{Machine Learning} \\
\midrule
\uncover<2->{\fixedrow{\textbf{Example counting}\\(seeing many repetitions)}}
  & \uncover<2->{\fixedrow{Rule-based systems / Traditional ML}} \\

\uncover<3->{\fixedrow{\textbf{Pointing out}\\(explicit correction / feedback)}}
  & \uncover<3->{\fixedrow{Supervised learning (labels, corrections)}} \\

\uncover<4->{\fixedrow{\textbf{Explaining in other words}\\(paraphrase / abstraction)}}
  & \uncover<4->{\fixedrow{Representation learning / Deep Learning}} \\

\uncover<5->{\fixedrow{\textbf{Interpreting context}\\(discourse / world knowledge)}}
  & \uncover<5->{\fixedrow{Transformers / Generative AI}} \\
\bottomrule
\end{tabularx}
\end{frame}
...
\begin{frame}{Evolution of Machine Learning}
\centering
\begin{tikzpicture}[scale=0.8, every node/.style={scale=0.9}]
  % Axes
  \draw[->,thick] (0,0) -- (9,0) node[right]{\textbf{Explicit $\;\to\;$ Black Box}};
  \draw[->,thick] (0,0) -- (0,4) node[above,rotate=90]{\textbf{Deceptively Simple $\;\to\;$ Deceptively Easy}};

  % Points along the diagonal trend
  \node[draw,rounded corners,fill=blue!20,align=center]   (rule) at (1,0.5) {Rule-based};
  \node[draw,rounded corners,fill=green!20,align=center]  (ml)   at (3,1.2) {Traditional\\ML};
  \node[draw,rounded corners,fill=orange!20,align=center] (dl)   at (5,2.0) {Deep\\Learning};
  \node[draw,rounded corners,fill=purple!20,align=center] (tr)   at (7,2.8) {Transformers};
  \node[draw,rounded corners,fill=red!20,align=center]    (gen)  at (8.8,3.5) {Generative\\AI};
\end{tikzpicture}
\end{frame}
...
\begin{frame}{From vector to matrix: Document Term Matrix (DTM)}
...
\pause
\emph{NOTE: The machine does not understand anything about the meaning of each word}
\end{frame}
...
\begin{frame}{Maybe we can do better?}
    What about:
    \begin{itemize}[<+->]
        \item negations: ``not left'' or ``didn't''
        \item context: ``bank'' near ``river'' or near ``money''
        \item synonyms: ``bike'' or ``bicycle''
        \item spelling mistakes: ``never'' or ``nevr''
        \item punctuation: ``did.'' or ``did''
        \item abbreviations: ``US'' vs ``United States'' vs ``U.S.''
        \item verb variations: ``biked'' vs ``bikes'' vs "biking"
        \item word derivations: ``beauty'' versus ``beautiful''
        \item non-informative words like ``a'', ``and'' or ``with''
        \item Capitalization: ``Tom, go please'' vs ``Please go, Tom''
        \item formatting symbols: ``\textbackslash n'' (newline) or ``<br>'' (HTML code)
    \end{itemize}
\end{frame}

\subsection{Fix 1: use weighting for informative vs less informative words}
...
\begin{align*}
  w_{i,j} &= tf_{i,j} \times \log \left(\frac{N}{df_{i}}\right) \\[1em]
  tf_{i,j} &= \text{ number of occurrences of term $i$ in document $j$} \\[0.5em]
  df_{i}  &= \text{ number of documents containing term $i$} \\[0.5em]
...
\subsection{Fix 2: Smart tokenization}
...
\subsection{-- Dealing with massive data intermezzo --}
...
\subsection{Fix 3: Preprocessing to deal with f.e. Capitalization, punctuation}
...
\subsection{Fix 4: Pruning of less informative words}
