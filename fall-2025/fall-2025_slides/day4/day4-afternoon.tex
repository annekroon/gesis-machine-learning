...
\begin{frame}{Reasons to do Cross-validation}
	\begin{block}{We do not ``waste'' too much validation data}
		\begin{itemize}
			\item If $k=10$, in each iteration, we use 90\% of the data for training.
...
\begin{itemize}
	\item introductory examples/pedagogical reasons
...
\begin{block}{Train/validation/test split}
	\begin{itemize}
		...
		\item Pedagogical reasons, simple start
...
\begin{frame}{The regularization parameter $\lambda$ (or $C = \frac{n}{\lambda}$)}
$$
...
\begin{itemize}[<+->]
	...
	\item (that's why the coefficients of a logistic regression classifier will differ from a ``traditional'' logistic regression with the statsmodels module)
	\item Remember that we are \textit{not} interested in a substantive interpretation of the  $\beta$-coefficients but in a good prediction (which is why ``punishing'' large coefficients and hence biasing the coefficients is not a problem at all!)
...
\begin{frame}{Hyperparameter optimization with grid search}
	\begin{block}{General idea}
		Rather than arbitrarily trying some combinations of hyperparameters, let's systematically test and compare.
	\end{block}
...
You might have noticed the Pipeline construct in the last example.
	\begin{itemize}
		...
		\item We did all of them separately before
...
\begin{itemize}[<+->]
	...
	\item Maybe it is important to us that our classifier is balanced and equally good in predicting both classes, even  if overall accuracy suffers (slightly) 
\end{itemize}
\pause
...
\begin{block}{More advanced pipelines}
    ...
    \url{https://scikit-learn.org/stable/auto_examples/compose/plot\_digits\_pipe.html}
\end{block}
...
\section{ML versus dictionaries}
...
\begin{frame}{In some sense, yes.}
	...
	\pause
	We \emph{could} print all coefficient-word pairs, but probably it's enough to just look at those with the largest absolute value:
\end{frame}


\subsection{The explainability of ML}
...
\begin{frame}{ELI5}
	\begin{itemize}
		...
		\item We can look which words in a given text contributed most to its classification
	\end{itemize}
\end{frame}


\begin{frame}{Have we solved all problems of dictionaries?}
	No.
