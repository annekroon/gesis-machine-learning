...
\usepackage{tabularx}
% helper: fixed-height centered cell content
\newcommand{\fixedrow}[1]{%
  \begin{minipage}[c][1,4cm][c]{\linewidth}\centering #1
  \end{minipage}}
...
\begin{frame}{How Humans Learn vs. How Machines Learn}
\small
\begin{tabularx}{\linewidth}{@{} >{\raggedright\arraybackslash}X  >{\raggedright\arraybackslash}X @{}}
\toprule
\textbf{Human Learning}  \\
\midrule
\fixedrow{\textbf{Example counting}\\(seeing many repetitions)} 
  

\fixedrow{\textbf{Pointing out}\\(explicit correction / feedback)} 
  

\fixedrow{\textbf{Explaining in other words}\\(paraphrase / abstraction)} 
  

\fixedrow{\textbf{Interpreting context}\\(discourse / world knowledge)} \\
  
\bottomrule
\end{tabularx}
\end{frame}

\begin{frame}{Lego pipeline}
...
\begin{frame}{Performance: what do we do and how to evaluate it}
\centering
\begin{animateinline}[controls]{5}
\multiframe{10}{i=0+1}{
    \begin{tikzpicture}[scale=1.2]
        % Draw negative points (red, lower half)
        \foreach \x/\y in {0/0, 1/0.5, 2/0.2, 2.5/0.8, 3/0.1, 0.5/1, 1.5/1.2, 2.2/0.7, 2.8/1.1, 3.2/0.6} {
            \fill[red] (\x,\y) circle (2pt);
        }
        % Draw positive points (blue, upper half)
        \foreach \x/\y in {0.2/2.2, 1/2.5, 2/2.8, 2.5/2.1, 3/2.7, 0.7/1.7, 1.8/1.9, 2.3/2.3, 2.9/1.8, 3.3/2.2} {
            \fill[blue] (\x,\y) circle (2pt);
        }
        % Calculate circle center: move from x=-1 to x=2, y=1.7
        \pgfmathsetmacro{\cx}{ifthenelse(\i<7,-1+(\i-1)*0.5,2)}
        \pgfmathsetmacro{\cy}{1.7}
        % Draw moving/stationary circle
        \draw[thick, dashed] (\cx,\cy) circle (1);
        % Highlight points inside the circle
        \foreach \x/\y/\col in {
            0/0/red, 1/0.5/red, 2/0.2/red, 2.5/0.8/red, 3/0.1/red, 0.5/1/red, 1.5/1.2/red, 2.2/0.7/red, 2.8/1.1/red, 3.2/0.6/red,
            0.2/2.2/blue, 1/2.5/blue, 2/2.8/blue, 2.5/2.1/blue, 3/2.7/blue, 0.7/1.7/blue, 1.8/1.9/blue, 2.3/2.3/blue, 2.9/1.8/blue, 3.3/2.2/blue
        } {
            \pgfmathsetmacro{\dist}{sqrt((\x-\cx)^2 + (\y-\cy)^2)}
            \ifdim \dist pt < 1pt
                \draw[fill=yellow,opacity=0.5] (\x,\y) circle (4pt);
            \fi
        }
        % Legend
        \draw[red] (3.7,2.8) circle (2pt); \node[right] at (3.8,2.8) {Negative};
        \draw[blue] (3.7,2.5) circle (2pt); \node[right] at (3.8,2.5) {Positive};
        \draw[thick, dashed] (3.7,2.1) circle (6pt); \node[right] at (3.9,2.1) {annotated positive};
    \end{tikzpicture}
}
\end{animateinline}
...
\begin{frame}{Example dictionary (Minqing Hu and Bing Liu, 2004)}
    \makebox[\linewidth]{
        \includegraphics[width=\linewidth,height=0.8\paperheight,keepaspectratio]{pictures/positive_words.jpg}
    }
\end{frame}
...
\question{Can we construct a dictionary that, \emph{irrespective of the context}, gives us a meaningful estimate of sentiment?}

%Such an \textit{off-the-shelf} dictionary does not (and probably cannot) exist.


%cut examples, focus on dictionaries and SML have their issues, no perfect performance
...
\begin{frame}{\cite{VanAtteveldt2021}: Extending \cite{Boukes2020} with SML}
	...
	Reaching $\alpha = 0.50$ for deep learning (CNN) and slightly worse for classical machine learning (SVM; $\alpha = 0.41$, NB; $\alpha = 0.40$), machine learning still performs significantly better than chance [\ldots]
	
	Finally, [\ldots] dictionaries [\ldots] perform worse than the machine
	learning results and much worse than manual annotation [\ldots] [and] approximate chance agreement''
\end{frame}
...
\begin{frame}[standout]
	SML is no panacea, but it introduces a promising approach to analyzing large quantities of texts. Don't believe off-the-shelf packages that claim to do the work for you.
...
\begin{itemize}[<+->]
	...
	\item To avoid $p$-hacking-like scenarios (which we call ``overfitting''), there are techniques available (e.g., cross-validation, which we address this afternoon)
\end{itemize}
...
% skip details: focus on understanding mechanism: Bayes is about probabilities of a label given the feature and the prevalence of both features and the labels in the corpus
...
\subsection{A note on sample size}
 
\begin{frame}{Training size vs performance \cite{Burscher2015}}
    \makebox[\linewidth]{
        \includegraphics[width=0.95\paperwidth,height=0.8\paperheight,keepaspectratio]{burscher2015-a}
    }
\end{frame}
...
\begin{frame}{What does this mean for our research?}
	...
	\begin{itemize}
	   ...
	   \item with an acceptable accuracy (at least for some of them)
          \item Some easier tasks even need only 500 training documents, see \cite{Hopkins2010}      
	\end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{And it works!}
	...
	\begin{itemize}
		\item We created a list with 25,000 training tuples and another one with 25,000 test tuples and
